...
def studentRecord():
    ...
    myCursor.execute("""SELECT student.student_id, student.first_name, student.last_name, student.gender, course.course_name, student.year, student.image, course.course_code FROM course
    ...
    if request.method == 'POST':
        if request.files:
            ...
            if len(fname) == 0:
                ...
                myCursor.execute("INSERT INTO student(student_id, first_name, last_name, gender, course_code, year, image) "
                                "VALUES(%s, %s, %s, %s, %s, %s, %s)", (idnum, fname, lname, gender, course, yrlvl, url))
                mysql.connection.commit()
    ...
    if request.method == 'POST':
        if request.files:
            ...
            if len(fname) == 0:
                ...
                myCursor.execute("UPDATE student SET student.first_name=%s, student.last_name=%s, student.gender=%s, student.course_code=%s, student.year=%s, student.image=%s WHERE student.student_id=%s",
                    (fname, lname, gender, course, yrlvl, url, idnum))
                mysql.connection.commit()
